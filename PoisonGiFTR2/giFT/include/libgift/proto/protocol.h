/*
 * $Id: protocol.h,v 1.1 2003/10/27 21:52:27 jashton Exp $
 *
 ...
 * @file protocol.h
 *
 * @brief Provides a shared method definition for plugins and the daemon to
 *        use for all communication.
 *
 * The protocol structure unifies all the API that plugins will use to
 * interact directly with the daemon for clarity and portability reasons.
 */
...
/* provide LIBGIFTPROTO_VERSION and LIBGIFTPROTO_MKVERSION */
#include "protocol_ver.h"              /* generated by autoconf */
...
typedef struct protocol
{
	...
	char     *version_str;             /**< protocol version, will be shown
	                                    *   by giFT -V */
...
#define UPLOAD_AUTH_HIDDEN        (-5)
	 ...
	 * @param user
	 *
	 *   Username that can be compared with previously registered uploads.
	 *   Consider setting and using Protocol::user_cmp for this to work well.
	 *
	 * @param share      Share that we are considering for upload.
	 * @param auth_info
	 *
	 *   Optional interface for retrieving additional information from the
	 *   auth call, such as which queue position the request *has been placed
	 *   in for UPLOAD_AUTH_MAX.
	 *
	 ...
	 *
	 * @retval UPLOAD_AUTH_HIDDEN
	 *
	 *   The user is not sharing any files at the moment, but may share
	 *   in the future.
	 */
	int (*upload_auth) (struct protocol *p, const char *user,
	                    struct file_share *share, upload_auth_t *auth_info);
	 ...
	 *
	 * @param p
	 * @param chunk  Storage location for the chunk handle of the newly
	 *               created transfer.
	...
	/**
	 * Notify a protocol about a new source being added to a transfer.  This
	 * affects only downloads.
	 *
	 * @return Return FALSE if giFT should abort adding this source,
	 *         effectively ignoring that addsource request.  Please note that
	 *         at the time of this writing, a return value of FALSE is not
	 *         implemented for internal reasons.  If you encounter this to
	 *         still be the case, notify the developers and the feature will
	 *         be finished.
	 */
	BOOL (*source_add) (struct protocol *p,
	                    struct transfer *transfer, struct source *source);
	 ...
	 * the specified transfer (you may assume that this is a download), or
	 * when a transfer has completed all sources will be removed.
	 */
	void (*source_remove) (struct protocol *p,
	                       struct transfer *transfer, struct source *source);
...
/**
 * Eased interface to maintain backwards compatibility and hide the link
 * check.
 */
#define protocol_compat(p,dev_ver) \
 ...
 *
 * @param p
 * @param link_ver  Version known at compile time.  Always pass
 *                  ::LIBGIFTPROTO_VERSION.
 * @param dev_ver   Last known compatible libgiftproto version known to the
 *                  author.  Use the macro ::LIBGIFTPROTO_MKVERSION.
 *
 * @return If the return value is less than, equal to, or greater than zero
 *         the version supplied is respectively too low, compatible, or
 *         too great for the current libgiftproto runtime.
 */
int protocol_compat_ex (Protocol *p, uint32_t link_ver, uint32_t dev_ver);
